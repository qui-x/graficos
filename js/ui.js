(function (global) {
  ...
  const AppUI = {
    ...
    mathState:null,confirmResolver:null,a11yPrefs:null,viewPrefs:null,tourState:null,tourResizeHandler:null,editingColor:null,currentProjectName:'Projeto OrbisV',pendingImportProject:null,pendingImportFileName:'',
    ...
    bindCore(){
      ...
      idListen('multiDuplicateBtn','click',()=>{const reserved=[];for(const id of [...this.selectedIds]){const o=this.objects.getById(id);if(!o)continue;const color=this.nextColorForType(o.type,reserved);reserved.push(color);this.objects.duplicate(id,color);}this.clearSelection();});
    ...
    applySnap(p){const mode=this.viewPrefs.snap||'auto';const scale=Math.max(1,this.engine?.scale||1);if(mode==='off')return p;const snapToNotable=(point)=>{if(!this.engine?.notablePoints?.length)return null;let best=null,dist=Infinity;for(const n of this.engine.notablePoints){const d=Math.hypot(point.x-n.x,point.y-n.y);if(d<dist){best=n;dist=d;}}return best&&dist<Math.max(.18,7/scale)?{x:best.x,y:best.y}:null;};const snapToInteger=(point)=>{const tolerance=Math.max(.08,7/scale);const rx=Math.round(point.x),ry=Math.round(point.y);return{x:Math.abs(point.x-rx)<tolerance?rx:point.x,y:Math.abs(point.y-ry)<tolerance?ry:point.y};};const snapToGrid=(point)=>{const s=this.engine.gridStep();const tol=Math.max(s*.18,6/scale);const rx=Math.round(point.x/s)*s,ry=Math.round(point.y/s)*s;return{x:Math.abs(point.x-rx)<tol?rx:point.x,y:Math.abs(point.y-ry)<tol?ry:point.y};};if(mode==='notable')return snapToNotable(p)||p;if(mode==='integer')return snapToInteger(p);if(mode==='grid')return snapToGrid(p);if(mode==='auto'){const notable=snapToNotable(p);if(notable)return notable;const integer=snapToInteger(p);const grid=snapToGrid(integer);return grid;}return p;},
    ...
    openManual(){
      ...
      const saved=Number(localStorage.getItem('orbisvManualDockWidthV1'));if(Number.isFinite(saved)&&saved>=360&&!this.isMobile()&&global.innerWidth>1120){document.documentElement.style.setProperty('--manual-dock-w',`${Math.min(760,Math.max(360,saved))}px`);}
    ...
    bindManualResize(){const handle=this.$.manualDockResizer;if(!handle||this._manualResizeBound)return;this._manualResizeBound=true;let active=false;const move=(e)=>{if(!active||global.innerWidth<=1120)return;const width=Math.min(760,Math.max(360,global.innerWidth-e.clientX));document.documentElement.style.setProperty('--manual-dock-w',`${width}px`);localStorage.setItem('orbisvManualDockWidthV1',String(Math.round(width)));this.engine.resize();};const stop=()=>{if(!active)return;active=false;document.body.classList.remove('manual-resizing');this.$.manualDock?.classList.remove('resizing');global.removeEventListener('pointermove',move);global.removeEventListener('pointerup',stop);};handle.addEventListener('pointerdown',(e)=>{if(global.innerWidth<=1120)return;e.preventDefault();active=true;document.body.classList.add('manual-resizing');this.$.manualDock?.classList.add('resizing');global.addEventListener('pointermove',move);global.addEventListener('pointerup',stop,{once:true});});},
    ...
    tourSteps(){return[
      {title:'Bem-vindo ao OrbisV',text:'Esta é a tela inicial do programa. Sempre que o OrbisV for aberto, você poderá continuar a sessão, criar ou abrir projetos, acessar modelos, exportação, acessibilidade, manual didático e este tour.',target:()=>document.getElementById('projectSheet'),prepare:()=>this.openProjectHome()},
      ...
      {title:'Manual didático',text:'O Manual abre em uma janela integrada ao OrbisV. No desktop ele pode ficar lado a lado com o gráfico; no celular ocupa uma tela compacta dentro do aplicativo. Os botões Abrir no OrbisV levam diretamente ao recurso correspondente.',target:()=>document.getElementById('manualBtn'),prepare:()=>this.closeAllModals()},
    ...
    async exportAs(type){const base=this.safeBaseName(document.getElementById('exportBaseName')?.value||this.currentProjectName),status=document.getElementById('exportStatus');try{if(type==='project'){this.currentProjectName=document.getElementById('projectNameInput')?.value||this.currentProjectName;this.saveProjectMeta();const payload=this.buildProjectPayload();this.download(JSON.stringify(payload,null,2),'application/json',`${base}.orbisv`);if(status)status.textContent='Projeto completo exportado com sucesso.';this.showToast('Projeto exportado.');return;}if(type==='png'){const scale=Number(document.getElementById('exportPngScale')?.value||2);this.engine.exportPng(`${base}-grafico.png`,scale);if(status)status.textContent=`PNG exportado em ${scale}×.`;this.showToast('Imagem PNG exportada.');return;}if(type==='svg'){const result=this.engine.exportSvg(`${base}-grafico.svg`);if(status)status.textContent=result?.rasterized?'SVG exportado preservando a cena como imagem incorporada.':'SVG vetorial exportado.';this.showToast(result?.rasterized?'SVG exportado com cena incorporada.':'SVG vetorial exportado.');return;}if(type==='pdf'){this.exportPdfReport(base);if(status)status.textContent='Relatório aberto para impressão/salvamento em PDF.';return;}if(type==='csv'){this.exportCsv(base);if(status)status.textContent='Tabela CSV exportada.';return;}}catch(e){if(status)status.textContent=e.message||'Falha na exportação.';this.showToast(e.message||'Não foi possível exportar.',true);}},
    exportCsv(base='OrbisV-tabela'){const funcs=this.objects.visible.filter(o=>o.type==='function');if(!funcs.length)throw new Error('Adicione ao menos uma função visível para gerar a tabela CSV.');const mode=document.getElementById('exportCsvRangeMode')?.value||'view',b=this.engine.currentBounds();let xmin=b.xmin,xmax=b.xmax,step=(xmax-xmin)/120;if(mode==='custom'){xmin=Number(document.getElementById('exportCsvMin')?.value);xmax=Number(document.getElementById('exportCsvMax')?.value);step=Number(document.getElementById('exportCsvStep')?.value);}if(!Number.isFinite(xmin)||!Number.isFinite(xmax)||!Number.isFinite(step)||step<=0||xmax<=xmin)throw new Error('Defina um intervalo CSV válido e um passo maior que zero.');const rows=Math.floor((xmax-xmin)/step)+1;if(rows>20001)throw new Error('O intervalo produziria mais de 20.001 linhas. Aumente o passo.');const heads=['x',...funcs.map((o,i)=>`${o.name||`f${i+1}`} = ${o.data.expression}`)],data=[heads.map(v=>this.csvCell(v)).join(';')];const compiled=funcs.map(o=>{try{return MathEngine.compile(o.data.expression,{x:0});}catch{return null;}});for(let i=0;i<rows;i+=1){const x=i===rows-1?Math.min(xmax,xmin+i*step):xmin+i*step,line=[this.csvNumber(x)];compiled.forEach(fn=>{const y=fn?fn({x}):NaN;line.push(Number.isFinite(y)?this.csvNumber(y):'');});data.push(line.join(';'));}this.download('\ufeff'+data.join('\r\n'),'text/csv;charset=utf-8',`${base}-tabela.csv`);this.showToast(`${rows} linha(s) exportadas para CSV.`);},
