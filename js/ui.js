(function (global) {
  ...
  const AppUI = {
    ...
    async openProjectFile(file){if(!file)return;try{const text=await file.text(),project=JSON.parse(text);this.objects.importProject(project);if(Array.isArray(project.history))this.objects.events=deepClone(project.history).slice(0,this.objects.maxEvents||180);if(project.scene?.view)this.engine.setView(project.scene.view);if(project.ui?.mode&&MODE_META[project.ui.mode])this.setMode(project.ui.mode,false);this.viewPrefs={...this.viewPrefs,grid:this.engine.showGrid,minorGrid:this.engine.showMinorGrid,axes:this.engine.showAxes,labels:this.engine.showLabels,coords:this.engine.showCoordinates,pointValues:this.engine.showPointValues};this.syncViewControls();this.closeAllModals();this.showToast('Projeto aberto com sucesso.');}catch(e){this.showToast(e.message||'Não foi possível abrir o projeto.',true);}finally{this.$.fileInput.value='';}},
