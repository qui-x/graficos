(function (global) {
  ...
  const AppUI = {
    ...
    mathState:null,confirmResolver:null,a11yPrefs:null,viewPrefs:null,
    ...
    bindCore(){
      ...
      idListen('inspectBtn','click',(e)=>{this.engine.inspectMode=!this.engine.inspectMode;e.currentTarget.classList.toggle('active',this.engine.inspectMode);e.currentTarget.setAttribute('aria-pressed',String(this.engine.inspectMode));if(!this.engine.inspectMode){this.engine.inspectX=null;this.$.inspectCard.hidden=true;}else this.showToast('Modo Inspecionar ativo. Mova o ponteiro sobre o gráfico.');this.engine.requestRender();});
    ...
    setMode(mode,openSheet=true){
      ...
      const m=MODE_META[mode];this.$.modeLabel.textContent=m.label.toUpperCase();this.$.modeTitle.textContent=this.editingId?'Editar objeto':m.title;
    ...
    submitCurrent(){
      try{
        if(this.activeMode==='function')this.saveFunction();else if(this.activeMode==='parametric')this.saveParametric();else if(this.activeMode==='vector')this.saveVector();else if(this.activeMode==='geometry')this.saveGeometry();else if(this.activeMode==='washers')this.saveWashers();else{this.showToast('O motor 3D será definido após o primeiro teste.');return;}
    ...
    showNotablePoints(explicitObj){const obj=explicitObj||this.objects.getById(this.editingId)||this.objects.items.find(o=>o.type==='function');if(!obj||obj.type!=='function'){this.showToast('Selecione uma função para analisar.');return;}try{const b=this.engine.currentBounds(),roots=MathEngine.roots(obj.data.expression,b.xmin,b.xmax),ext=MathEngine.extrema(obj.data.expression,b.xmin,b.xmax);this.engine.notablePoints=[...roots.map(x=>({x,y:0,color:obj.color,kind:'raiz'})),...ext.map(p=>({...p,color:obj.color}))];this.engine.selectedId=obj.id;this.engine.requestRender();this.showToast(`${this.engine.notablePoints.length} ponto(s) notável(is) destacado(s).`);}catch(e){this.showToast(e.message,true);}},
    ...
    bindProject(){
      idListen('projectBtn','click',()=>this.openModal('projectSheet'));idListen('newProjectBtn','click',async()=>{const ok=await this.confirm(`A cena atual possui ${this.objects.items.length} objeto(s). O autosave continuará guardando a versão anterior para recuperação.`,'Criar novo gráfico?');if(ok){this.objects.clear(true);this.objects.events=[];this.objects.undoStack=[];this.objects.redoStack=[];this.objects.save();this.engine.center();this.cancelEdit();this.closeAllModals();}});
      idListen('openProjectBtn','click',()=>this.$.fileInput.click());this.$.fileInput.addEventListener('change',(e)=>this.openProjectFile(e.target.files?.[0]));idListen('saveProjectBtn','click',()=>this.saveProjectFile());idListen('openExportBtn','click',()=>this.openModal('exportSheet'));
    ...
    loadPrefs(){const defaults={theme:'dark',highContrast:false,fontScale:1,letterSpacing:false,reduceMotion:false,simpleReading:false,colorVision:'none',linePatterns:true,markers:true,graphDescription:true,keyboardNav:true,nativeKeyboard:false};try{this.a11yPrefs={...defaults,...JSON.parse(localStorage.getItem(STORAGE_A11Y)||'{}')};}catch{this.a11yPrefs=defaults;}const view={grid:true,minorGrid:true,axes:true,labels:true,equalScale:true,coords:true,snap:'off'};try{this.viewPrefs={...view,...JSON.parse(localStorage.getItem(STORAGE_VIEW)||'{}')};}catch{this.viewPrefs=view;}},
    saveA11y(){localStorage.setItem(STORAGE_A11Y,JSON.stringify(this.a11yPrefs));},saveViewPrefs(){this.viewPrefs={grid:this.engine.showGrid,minorGrid:this.engine.showMinorGrid,axes:this.engine.showAxes,labels:this.engine.showLabels,equalScale:true,coords:this.engine.showCoordinates,snap:this.viewPrefs.snap||'off'};localStorage.setItem(STORAGE_VIEW,JSON.stringify(this.viewPrefs));},
    applyA11y(){const p=this.a11yPrefs,root=document.documentElement,allowedVision=new Set(['none','protanopia','deuteranopia','tritanopia','achromatopsia']);root.dataset.theme=p.theme==='light'?'light':'dark';root.dataset.colorVision=allowedVision.has(p.colorVision)?p.colorVision:'none';root.classList.toggle('high-contrast',Boolean(p.highContrast));root.classList.toggle('reduce-motion',Boolean(p.reduceMotion));document.body.classList.toggle('simple-reading',Boolean(p.simpleReading));document.body.classList.toggle('letter-spaced',Boolean(p.letterSpacing));root.style.setProperty('--font-scale',p.fontScale);root.style.colorScheme=root.dataset.theme;const themeMeta=document.getElementById('themeColorMeta');if(themeMeta)themeMeta.content=root.dataset.theme==='light'?(p.highContrast?'#ffffff':'#f4f8fc'):(p.highContrast?'#000000':'#07111e');this.syncA11yControls();this.renderObjects?.();this.engine?.requestRender();},
