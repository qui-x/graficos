(function (global) {
  ...
  class GraphEngine {
    ...
    exportSvg(filename='OrbisV-grafico.svg'){
      ...
      const{w,h}=this.size,theme=this.theme;
      if(unsupported){
        const png=this.snapshotDataUrl(2),desc=`OrbisV — cena ${this.viewMode==='3d'?'3D':'com objetos rasterizados'}`;
        const svg=`<svg xmlns="http://www.w3.org/2000/svg" width="${w}" height="${h}" viewBox="0 0 ${w} ${h}"><title>OrbisV</title><desc>${desc}</desc><image href="${png}" x="0" y="0" width="${w}" height="${h}" preserveAspectRatio="none"/></svg>`;
