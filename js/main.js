(function () {
  ...
  if ('serviceWorker' in navigator && location.protocol !== 'file:') {
    window.addEventListener('load', async () => {
      try {
        const registration = await navigator.serviceWorker.register('./sw.js?v=4.8.0', { scope: './', updateViaCache: 'none' });
        registration.update().catch(()=>{});
        let reloadedForUpdate = false;
        navigator.serviceWorker.addEventListener('controllerchange', () => {
          if (reloadedForUpdate || sessionStorage.getItem('orbisv-sw-reloaded-v4.8.0') === '1') return;
          reloadedForUpdate = true;
          sessionStorage.setItem('orbisv-sw-reloaded-v4.8.0','1');
          location.reload();
        });
      } catch (error) {
